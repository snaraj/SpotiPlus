...
def get_artist_summary(artist: str) -> str:
	...
	try:
		response = requests.get(f'http://ws.audioscrobbler.com/2.0/?method=artist.getinfo&artist={artist}&api_key={LASTFM_API_KEY}&format=json')
		if response == '':
			return summary
...
def get_track_summary(track: str, artist: str) -> str:
	...
	try:
		response = requests.get(f'http://ws.audioscrobbler.com/2.0/?method=track.getInfo&api_key={LASTFM_API_KEY}&artist={artist}&track={track}&format=json')
